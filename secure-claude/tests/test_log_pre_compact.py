...
SCRIPT = PLUGIN_ROOT / "hooks" / "scripts" / "log_pre_compact.py"
...
def test_logs_trigger_manual(tmp_path):
    r = _run({"cwd": str(tmp_path), "trigger": "manual"}, tmp_path)
    assert r.returncode == 0, r.stderr
    rec = _last_rec(tmp_path, tmp_path)
    assert rec["event"] == "preCompact"
    assert rec["trigger"] == "manual"


def test_logs_trigger_auto(tmp_path):
    r = _run({"cwd": str(tmp_path), "trigger": "auto"}, tmp_path)
    assert r.returncode == 0, r.stderr
    rec = _last_rec(tmp_path, tmp_path)
    assert rec["trigger"] == "auto"


def test_missing_trigger_defaults_to_unknown(tmp_path):
    r = _run({"cwd": str(tmp_path)}, tmp_path)
    assert r.returncode == 0, r.stderr
    rec = _last_rec(tmp_path, tmp_path)
    assert rec["trigger"] == "unknown"
