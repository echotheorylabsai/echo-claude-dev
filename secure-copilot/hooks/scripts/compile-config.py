...
VALID_TOOLS = {"bash", "edit", "create"}
VALID_TARGET_FIELDS = {"command", "path"}
VALID_ACTIONS = {"deny"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compile secure-copilot YAML rule config into runtime JSON")
    parser.add_argument("--mode", choices=["shipped", "local"], required=True)
    parser.add_argument("--check", action="store_true", help="Verify generated JSON is up to date instead of writing it")
    parser.add_argument("--plugin-root", help="Override the secure-copilot plugin root")
    ...
    parser.add_argument("--indirect-source", help="Override the indirect injection YAML source file")
...
def resolve_plugin_root(args: argparse.Namespace) -> Path:
    ...
    env_root = os.environ.get("SECURE_COPILOT_PLUGIN_ROOT", "")
    if env_root:
...
def normalize_prompt_defaults(defaults: dict[str, Any], label: str) -> dict[str, Any]:
    ...
    if "caseInsensitive" in defaults:
        normalized["caseInsensitive"] = require_bool(defaults["caseInsensitive"], f"{label}.caseInsensitive")
...
def normalize_tool_defaults(defaults: dict[str, Any], label: str) -> dict[str, Any]:
    ...
    if "tool" in defaults:
        ...
        if tool not in VALID_TOOLS:
            raise ConfigError(f"{label}.tool: unsupported tool {tool}")
    ...
    if "targetField" in defaults:
        ...
        if target_field not in VALID_TARGET_FIELDS:
            raise ConfigError(f"{label}.targetField: unsupported targetField {target_field}")
...
def compile_shipped_rules(
    ...
    global_defaults = normalize_defaults_fn(ensure_mapping(document.get("defaults"), f"{label}.defaults"), f"{label}.defaults")
    ...
    for group_name, group_value in groups.items():
        ...
        for index, raw_rule in enumerate(ensure_list(group_mapping.get("rules"), f"{group_label}.rules")):
...
def compile_shipped_prompt_rules(document: dict[str, Any], label: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    return compile_shipped_rules(document, "categories", normalize_prompt_defaults, normalize_prompt_rule, label)


def compile_shipped_tool_rules(document: dict[str, Any], label: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    return compile_shipped_rules(document, "groups", normalize_tool_defaults, normalize_tool_rule, label)
...
def compile_overrides(
    ...
    for index, raw_rule in enumerate(raw_rules):
        ...
        if normalized["id"] in shipped_ids:
            raise ConfigError(f"{rule_label}: override may not modify shipped rule id: {normalized['id']}")
        rules.append(normalized)
    validate_unique_ids(rules, f"{label}.{key}")
    return rules


def compile_prompt_overrides(document: dict[str, Any], defaults: dict[str, Any], shipped_ids: set[str], label: str) -> list[dict[str, Any]]:
    return compile_overrides(document, "promptPatterns", defaults, shipped_ids, normalize_prompt_rule, label)


def compile_tool_overrides(document: dict[str, Any], defaults: dict[str, Any], shipped_ids: set[str], label: str) -> list[dict[str, Any]]:
    return compile_overrides(document, "toolRules", defaults, shipped_ids, normalize_tool_rule, label)
...
def compile_shipped(args: argparse.Namespace, plugin_root: Path) -> None:
    prompt_source = Path(args.prompt_source).expanduser().resolve() if args.prompt_source else plugin_root / "hooks/config/user-prompt-threats.yaml"
    tool_source = Path(args.tool_source).expanduser().resolve() if args.tool_source else plugin_root / "hooks/config/tool-rules.yaml"
    indirect_source = Path(args.indirect_source).expanduser().resolve() if args.indirect_source else plugin_root / "hooks/config/indirect-injection-patterns.yaml"
    output_dir = Path(args.output_dir).expanduser().resolve() if args.output_dir else plugin_root / "hooks/config/generated"

    prompt_rules, _ = compile_shipped_prompt_rules(load_yaml_file(prompt_source, "prompt rules"), "prompt rules")
    tool_rules, _ = compile_shipped_tool_rules(load_yaml_file(tool_source, "tool rules"), "tool rules")

    write_or_check_output(output_dir / "user-prompt-threats.json", render_json(prompt_rules), args.check, "prompt rules")
    write_or_check_output(output_dir / "tool-rules.json", render_json(tool_rules), args.check, "tool rules")

    if indirect_source.exists():
        indirect_rules, _ = compile_shipped_prompt_rules(load_yaml_file(indirect_source, "indirect injection rules"), "indirect injection rules")
        write_or_check_output(output_dir / "indirect-injection-patterns.json", render_json(indirect_rules), args.check, "indirect injection rules")
...
def compile_local(args: argparse.Namespace, plugin_root: Path) -> None:
    ...
    tool_source = Path(args.tool_source).expanduser().resolve() if args.tool_source else plugin_root / "hooks/config/tool-rules.yaml"
    indirect_source = Path(args.indirect_source).expanduser().resolve() if args.indirect_source else plugin_root / "hooks/config/indirect-injection-patterns.yaml"
    override_source = Path(args.override_source).expanduser().resolve() if args.override_source else Path.home() / ".config/secure-copilot/overrides.yaml"
    output_dir = Path(args.output_dir).expanduser().resolve() if args.output_dir else Path.home() / ".config/secure-copilot/generated"

    shipped_prompt_rules, prompt_defaults = compile_shipped_prompt_rules(load_yaml_file(prompt_source, "prompt rules"), "prompt rules")
    shipped_tool_rules, tool_defaults = compile_shipped_tool_rules(load_yaml_file(tool_source, "tool rules"), "tool rules")

    # Collect shipped indirect injection rule IDs for override protection
    shipped_indirect_ids: set[str] = set()
    if indirect_source.exists():
        shipped_indirect_rules, indirect_defaults = compile_shipped_prompt_rules(load_yaml_file(indirect_source, "indirect injection rules"), "indirect injection rules")
    ...
    ensure_unknown_keys_absent(override_document, {"promptPatterns", "toolRules", "indirectInjectionRules"}, "local overrides")
    ...
    indirect_rules_list = ensure_list(override_document.get("indirectInjectionRules"), "local overrides.indirectInjectionRules")
    ...
    write_or_check_output(indirect_output, render_json(indirect_rules), False, "local indirect injection overrides")
...
def main() -> int:
    ...
    try:
        ...
        print(f"compile-config: {exc}", file=sys.stderr)
        return 1
    return 0
